...
def test_cpu_fp16_config_is_rejected():
    with pytest.raises(ValueError, match="fp16 mixed precision requires device='cuda'"):
        TrainingRunConfig.model_validate({"training": {"device": "cpu", "mixed_precision": "fp16"}})
